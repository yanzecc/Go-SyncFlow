目前测试无问题：
1.新员工加入钉钉架构后，自动通过工作通知发送账号和初始密码（消息模版可编辑）
2.将服务内置至钉钉工作台，员工在钉钉内SSO免登，可自助修改密码和忘记密码找回
3.平台内置LDAP服务（默认支持samba），可接入群晖，gitlab，truenas等
4.账号密码的属性更改时，触发事件同步，可自动同步至AD域。
5.钉钉同步至本地，当员工离职时，自动禁用本地账号。当账号被禁用时，可选下游映射策略，将ad账号禁用或删除可自定义。
